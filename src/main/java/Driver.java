...
//        String uriWithLink = filePath.toUri().toString();
        ...
        //job6.setNumReduceTasks(1);
        job7.setMapperClass(Job6Mapper2.class);
        //job7.setSortComparatorClass(Job7Comparator.class);
        //job6.setCombinerClass(Job6Reducer1.class);
        job7.setReducerClass(Job6Reducer2.class);

        job7.setOutputKeyClass(Text.class);
        job7.setOutputValueClass(Text.class);
        FileInputFormat.addInputPath(job7, new Path("/job6_tmp"));
        FileOutputFormat.setOutputPath(job7, new Path("/Job6"));
        job7.waitForCompletion(true);


        /*******Driver code of Job7********///(Weather-related delay for each city.)
        Job job8 = Job.getInstance(conf, "CS455-PA3");
        job8.setJarByClass(Driver.class);
        //job6.setNumReduceTasks(1);
        job8.setMapperClass(Job7Mapper1.class);
        //job1.setSortComparatorClass(Job1Comparator.class);
        //job6.setCombinerClass(Job6Reducer1.class);
        job8.setReducerClass(Job7Reducer1.class);
        //job6.setMapOutputKeyClass(Text.class);
        //job6.setMapOutputValueClass(IntWritable.class);
        job8.setOutputKeyClass(Text.class);
        job8.setOutputValueClass(Text.class);
        FileInputFormat.addInputPath(job8, new Path(args[0]));
        FileOutputFormat.setOutputPath(job8, new Path("/job7_tmp"));
...
//        String uriWithLink = filePath.toUri().toString();
        ...
        DistributedCache.addCacheFile(new URI("main/data/supplementary/plane-data.csv"), conf);
        DistributedCache.addCacheFile(new URI("main/data/supplementary/carriers.csv"), conf);
        Job job8_1 = Job.getInstance(conf, "CS455-PA3");
        job8_1.setJarByClass(Driver.class);
        //job6.setNumReduceTasks(1);
        job8_1.setMapperClass(Job7Mapper2.class);
        //job7.setSortComparatorClass(Job7Comparator.class);
        //job6.setCombinerClass(Job6Reducer1.class);
        job8_1.setReducerClass(Job7Reducer2.class);

        job8_1.setOutputKeyClass(Text.class);
        job8_1.setOutputValueClass(Text.class);
        FileInputFormat.addInputPath(job8_1, new Path("/job7_tmp"));
        FileOutputFormat.setOutputPath(job8_1, new Path("/Job7"));
        System.exit(job8_1.waitForCompletion(true)? 0: 1);
