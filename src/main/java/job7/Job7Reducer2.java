...
public class Job7Reducer2 extends Reducer<Text,Text,Text,Text> {
    ...
    public void reduce (Text key, Iterable < Text > values,
        ...
        for (Text result : values) {
            ...
            Integer numberOfDelay = Integer.parseInt(elements[1]);
            Integer totalDelay = Integer.parseInt(elements[2]);
            Float avgDelay = Float.parseFloat(elements[3]);

            airportToCountDelayMap.put(airportID, numberOfDelay);
            airportToSumDelayMap.put(airportID, totalDelay);
            airportToAvgDelayMap.put(airportID, avgDelay);
        }
        ...
        context.write(null, new Text("Top 10 Airports/Cities with most diversion related issues:"));
        Set<String> keys = sortedCountDelay.keySet();
        String[] keysArray = keys.toArray(new String[keys.size()]);
        for (int i = 0; i < keysArray.length && i < 10; i++) {
            context.write(new Text(Integer.toString(i + 1) + ". " + airportLookupTable.get(keysArray[i])), new Text("\t\t\tNumber of times delay occured: "+Integer.toString(sortedCountDelay.get(keysArray[i]))+" and Carrier Details: "+carrierLookupTable.get(keysArray[i])));
        }
        //System.out.println("The value of lookup is: "+airportLookupTable.get("ORD"));
        context.write(null, new Text("\n"));
        context.write(null, new Text("Top 10 Airports/Cities with highest sum total diversion-realted delays:"));
        ...
        for (int i = 0; i < keysArray1.length && i < 10; i++) {
            ...
            context.write(new Text(Integer.toString(i + 1) + ". " + airportLookupTable.get(keysArray1[i])), new Text("\t\t\tTotal delays in minutes: "+Integer.toString(sortedTotalDelay.get(keysArray1[i]))+" and Carrier Details: "+carrierLookupTable.get(keysArray1[i])));
        }

        context.write(null, new Text("\n"));
        context.write(null, new Text("Top 10 Airports/Cities with highest average diversion-realted delays:"));
        ...
        for (int i = 0; i < keysArray2.length && i < 10; i++) {
            context.write(new Text(Integer.toString(i + 1) + ". " + airportLookupTable.get(keysArray2[i])), new Text("\t\t\tAvegrage Delay in minutes: "+Float.toString(sortedAvgDelay.get(keysArray2[i]))+" and Carrier Details: "+carrierLookupTable.get(keysArray2[i])));
        }
    }
}
